...
def test_eventbridge():
    ...
    try:
        events_client = boto3.client(
            "events", endpoint_url=ENDPOINT_URL, region_name=REGION
        )

        # Try to list rules
        print("\n1. Testing list_rules()...")
        response = events_client.list_rules()
        print("✓ EventBridge is available!")
...
def test_stepfunctions():
    ...
    try:
        ...
        print("✓ Step Functions is available!")
...
def check_localstack_services():
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            print(
                f"  {'✓' if services.get('events') == 'running' else '✗'} events (EventBridge): {services.get('events', 'not available')}"
            )
            print(
                f"  {'✓' if services.get('stepfunctions') == 'running' else '✗'} stepfunctions: {services.get('stepfunctions', 'not available')}"
            )
            print(
                f"  {'✓' if services.get('lambda') == 'running' else '✗'} lambda: {services.get('lambda', 'not available')}"
            )
            print(
                f"  {'✓' if services.get('iam') == 'running' else '✗'} iam: {services.get('iam', 'not available')}"
            )
            print(
                f"  {'✓' if services.get('cloudformation') == 'running' else '✗'} cloudformation: {services.get('cloudformation', 'not available')}"
            )
...
if __name__ == "__main__":
    ...
    print("=" * 80)
    print(
        f"EventBridge:   {'✓ AVAILABLE' if eventbridge_available else '✗ NOT AVAILABLE'}"
    )
    print(
        f"Step Functions: {'✓ AVAILABLE' if stepfunctions_available else '✗ NOT AVAILABLE'}"
    )
    ...
    if eventbridge_available and stepfunctions_available:
        ...
        print(
            "  You can still use Step Functions manually, but scheduled execution won't work."
        )
    else:
        print("\n✗ One or both services are not available.")
        print(
            "  Consider using the Python workflow simulation (run_workflow.py) instead."
        )
