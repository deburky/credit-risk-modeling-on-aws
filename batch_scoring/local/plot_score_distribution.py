...
Uses CatBoost model with SHAP values converted via PDO method
"""
...
model = joblib.load("model_output/catboost_model.joblib")
...
y = 1 - df["Good_Bad"]  # Reverse: 0=good, 1=bad
...
log_odds = feature_shap.sum(axis=1) + base_shap

# Convert to scores using PDO formula
...
plt.title("CatBoost SHAP Values Scorecard", fontsize=18, fontweight="bold", pad=15)
